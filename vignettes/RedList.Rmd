...
| `.$workflowOutput()`  | Specify the output of the workflow.                                                                        |
| `.$obtainMeta()`      | Obtain metadata for the occurrence records.                                                                |
>>>>>>> cf2988f08cf438f91df6ad2020c55bbd5ee9f384
...
We also specify priors for the intercept terms and the fixed effects of the models. In this vignette we choose tight priors (mean = 0; precision = 5).
...
workflow$specifyPriors('tavg', Mean = 0, Precision = 5)
...
workflow$biasFields('CZ', 
                    prior.range = c(1, 0.1), #1 #0.1
                    prior.sigma = c(0.1, 0.1), #1 #0.1
                    constr = FALSE)
